...
def get_current_stuff(room, player):
    ...
    for person in room.new_people:
        if room.new_people[person].get("exists","begin") not in player.events:
            continue
        if person in people:
...
if __name__ == "__main__":
    ...
    room = rooms[current_place]
    goto_room(current_place)
    c = ""
    while True:
        print(f"room is actually {room}")
        options = get_current_stuff(room, player)
        interact(options, room)
